...
export interface GenerateEmailTemplateParams extends EmailTemplateData {
  daysLeft: number;
}
...
export const emailTemplates: EmailTemplate[] = [
  {
    ...
    generateSubject: (data: EmailTemplateData) =>
      `📅 Reminder: Your ${data.subscriptionName} Subscription Renews in 7 Days!`,
    generateBody: (data: EmailTemplateData) => generateEmailTemplate({ ...data, daysLeft: 7 }),
  },
  {
    label: "5 days before reminder",
    generateSubject: (data: EmailTemplateData) =>
      `⏳ ${data.subscriptionName} Renews in 5 Days – Stay Subscribed!`,
    generateBody: (data: EmailTemplateData) => generateEmailTemplate({ ...data, daysLeft: 5 }),
  },
  {
    label: "2 days before reminder",
    generateSubject: (data: EmailTemplateData) =>
      `🚀 2 Days Left!  ${data.subscriptionName} Subscription Renewal`,
    generateBody: (data: EmailTemplateData) => generateEmailTemplate({ ...data, daysLeft: 2 }),
  },
  {
    label: "1 days before reminder",
    generateSubject: (data: EmailTemplateData) =>
      `⚡ Final Reminder: ${data.subscriptionName} Renews Tomorrow!`,
    generateBody: (data: EmailTemplateData) => generateEmailTemplate({ ...data, daysLeft: 1 }),
  },
];